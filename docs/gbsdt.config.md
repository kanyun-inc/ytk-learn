...
data {
    ...
    delim {
        # separate sample weight, labels, features, init_prediction(only in tree model)
        x_delim : "###",
    ...
    # files_avg : different slaves read different files, if your files outnumber slaves, and the number of samples in each file is similar, we recommend the following manner
...
model {
    # path for model parameters(checkpoint path), format:
...
loss {
    ...
    # whether you only want to evaluate some metrics(training finished)
    just_evaluate : false,
  
    # model parameters regularization to avoid overfitting,
    # l1 and l2 can be used at the same time(elastic net)
    regularization : {
        # l1 regularization(parameters with laplace distribution prior),
        # single variable score and gating l1 regular
        l1 : [5.28e-9, 5.28e-9],    
        # l2 regularization(parameters with guassian distribution prior),
        # single variable score and gating l2 regular
        l2 : [5.28e-5, 5.28e-7] 
    }
},
...
optimization {
    ...
    line_search {
        ...
        # if you use "wolfe" mode, loss will always decrease whether loss is convex or not
        mode : "wolfe",
...
hyper {
    ...
    hoag {
        ...
        # between two continuous steps, if gradient is opposite, step will decrese, step = step * step_decr_factor 
    ...
    grid {
        ...
        # first three terms are used for single variable score, last three terms are used for gating
...
random {
    # random initialization method, includes normal/uniform distribution(excluded single variable score params)
    mode : "normal",
...
# in logistic regression, recommended value: pos / (pos + neg) 
...
# variable value score initialization range
# binary classification task, [-2, 2] recommended.
# regression task, [min_label_value, max_label_value] recommended.
leaf_random_init_range : [-2, 2], 

# there are two kinds of tree:  "gradient_boosting",  "random_forest",
# if you use "random_forest", learning_rate will be set 1.0 forcibly.
